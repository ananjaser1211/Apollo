...
#include <sound/sec_adaptation_richtek.h>
#include "abox.h"
...
int richtek_spm_read(void *buf, int size)
{
	...
	ret = abox_request_ipc(&data->pdev_abox->dev, IPC_ERAP, &msg,
			       sizeof(msg), 0, 0);

	if (ret < 0) {
		ipc_err("%s abox_request_ipc is failed: %d", __func__, ret);
		/*return -1;*/
	}
...
static irqreturn_t abox_adaptation_handler(int irq,
	...
	case IPC_ERAP:
		...
		case REALTIME_EXTRA:
			...
			case 0xffffffff:
				...
				break;
			default:
				ipc_err("unknown response type, RES_ID = %d",
					res_id);
				break;
			}
			break;
