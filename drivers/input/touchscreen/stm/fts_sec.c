...
static DEVICE_ATTR(edge_pos, 0444, fts_edge_x_position, NULL);
				...
				struct device_attribute *attr, char *buf)
{
	...
	input_info(true, &info->client->dev, "%s: %d %d %d\n",
				...
				info->scrub_x, info->scrub_y);
	snprintf(buff, sizeof(buff), "%d %d %d",
			info->scrub_id, info->scrub_x,
			info->scrub_y);
...
// Need to change function for sysinfo
static int fts_get_channel_info(struct fts_ts_info *info)
{
	...
	// Read Sense Channel
	rc = info->fts_get_sysinfo_data(info,
			FTS_SI_SENSE_CH_LENGTH,
			3, data);
	if (rc <= 0) {
		info->SenseChannelLength = 0;
		input_info(true, info->dev,
				"%s: Get channel info Read Fail!! [Data : %2X]\n",
				__func__, data[0]);
		return rc;
	}
...
void fts_print_frame(struct fts_ts_info *info, short *min, short *max)
{
	...
	for (i = 0; i < info->ForceChannelLength; i++) {
		...
		for (j = 0; j < info->SenseChannelLength; j++) {
			...
			if (i > 0) {
				if (info->pFrame[(i * info->SenseChannelLength) + j]
						< *min)
					*min =
...
int fts_read_frame(struct fts_ts_info *info,
		unsigned char type,
		short *min,
		short *max)
{
	unsigned char pFrameAddress[8] = {0xD0, 0x00, 0x00, 0x04,
		0x00, 0x00, 0x00, 0x00};
	...
	totalbytes =
		info->SenseChannelLength * info->ForceChannelLength * 2;
	ret = fts_read_reg(info, &pFrameAddress[0],
			3, pRead, pFrameAddress[3]);

	if (ret >= 0) {
		FrameAddress = pRead[1] + (pRead[2] << 8);

		start_addr =
			FrameAddress+info->SenseChannelLength * 2;
		end_addr =
	...
	remained = totalbytes;
	for (writeAddr = start_addr; writeAddr
			< end_addr; writeAddr
			+= READ_CHUNK_SIZE) {
		...
		fts_read_reg(info, &pFrameAddress[0],
				3, pRead, readbytes + 1);
		remained -= readbytes;
	...
	while (fts_read_reg(info, &cmd, 1,
		...
		if ((data[0] == 0x0F) && (data[1] == 0x05)) {
			...
			case 0x05:
				input_info(true,
						&info->client->dev,
						"[FTS] ITO Test result : Force channel [%d] short to VDD.\n",
					data[3]);
				break;
			case 0x06:
				input_info(true,
						&info->client->dev,
						"[FTS] ITO Test result : Sense channel [%d] short to VDD.\n",
					data[3]);
				break;
			case 0x07:
				input_info(true,
...
static void get_threshold(void *device_data)
{
	...
	if (rc <= 0) {
		input_info(true, info->dev,
				"%s: Get threshold Read Fail!! [Data : %2X%2X]\n",
				__func__,
				data[0], data[1]);
		sec->cmd_state = SEC_CMD_STATUS_FAIL;
		return;
...
static void get_checksum_data(void *device_data)
{
	...
	rc = info->fts_get_sysinfo_data(info,
			FTS_SI_CONFIG_CHECKSUM, 5, data);
	if (rc <= 0) {
		input_info(true, info->dev,
				"%s: Get checksum data Read Fail!! [Data : %2X%2X%2X%2X]\n",
...
static void run_delta_read(void *device_data)
{
	...
	struct fts_ts_info *info =
		container_of(sec, struct fts_ts_info, sec);
	char buff[SEC_CMD_STR_LEN] = { 0 };
	short min = 0x7FFF;
	short max = 0x8000;
...
static void get_strength_all_data(void *device_data)
{
	...
	memset(all_strbuff, 0,
			sizeof(char)*((info->ForceChannelLength)*(info->SenseChannelLength)*5)); //size 5  ex(1125,)
...
static void fts_read_self_raw_frame(struct fts_ts_info *info, unsigned short oAddr, bool allnode)
{
	...
	FrameAddress =
		ReadData[D0_offset] + (ReadData[D0_offset + 1] << 8);
	// D1 : DOFFSET = 0, D2 : DOFFSET : 1
	...
	if (mbuff != NULL) {
		...
		if (allnode == true) {
			...
			snprintf(buff, sizeof(buff), "%d,%d,%d,%d",
					min_tx_self_raw_data, max_tx_self_raw_data,
					min_rx_self_raw_data, max_rx_self_raw_data);
			sec->cmd_state = SEC_CMD_STATUS_OK;
		}
		sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
		input_info(true, &info->client->dev, "%s: %s\n", __func__, buff);
...
static void get_status_pureautotune(void *device_data)
{
	...
	if (rc <= 0) {
		input_info(true, info->dev,
				"%s: PureAutotune Information Read Fail!! [Data : %2X%2X]\n",
...
static void run_key_cx_data_read(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%d,%d,%d,%d",
			...
			total_cx_data[0], total_cx_data[1]);
...
int fts_get_tsp_flash_data(struct fts_ts_info *info, int offset,
		unsigned char cnt, unsigned char *buf)
{
...
void get_pat_data(struct fts_ts_info *info)
{
	...
	input_info(true, &info->client->dev,
			"cal_count = [0x%X], tune_fix_ver = [0x%4X]\n",
			info->cal_count, info->tune_fix_ver);
...
static void set_lpgw_mode(void *device_data)
{
	...
	if (sec->cmd_param[0] < 0 || sec->cmd_param[0] > 1) {
		...
		if ((sec->cmd_param[0] && (info->lowpower_flag & FTS_LOWP_FLAG_GESTURE_WAKEUP))
			...
			input_info(true, &info->client->dev,
					"%s: duplicate lpgw mode setting cmd:[0x%x], lowpower_flag:[0x%X]\n",
					__func__, sec->cmd_param[0],
					info->lowpower_flag & FTS_LOWP_FLAG_GESTURE_WAKEUP);
		} else {
			if (sec->cmd_param[0]) {
	...
	for (i = 0; i < 4; i++) {
		if (sec->cmd_param[i]) {
			...
			if (ret < 0) {
				...
				input_dbg(true, &info->client->dev,
						"%s: write tunning data:%d, %d, %d, ret:%d\n",
						__func__,
...
static void run_force_calibration(void *device_data)
{
	...
	if (touch_on) {
		...
		if (info->cal_count > PAT_MAX_LCIA)
...
static DEVICE_ATTR(brightness, 0664, NULL, fts_touchkey_led_control);
