...
static void hx83102_chip_init(void)
{
	...
	(*kp_PANEL_VERSION_ADDR)		= 49156;  /*0x00C004*/
...
static void hx83102e_chip_init(void)
{
	...
	(*kp_CID_VER_MIN_FLASH_LENG)	= 1;
	(*kp_PANEL_VERSION_ADDR)		= 59396;  /*0x00E804*/
	(*kp_PANEL_VERSION_LENG)		= 1;
...
static void hx83102ab_set_SMWP_enable(uint8_t SMWP_enable)
{
	uint8_t tmp_data[DATA_LEN_4];
	...
	do {
		if (SMWP_enable) {
			kp_himax_in_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
			kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_smwp_enable, DATA_LEN_4, tmp_data, 0);
			kp_himax_in_parse_assign_cmd(fw_func_handshaking_pwd, back_data, 4);
		} else {
			kp_himax_in_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, tmp_data, 4);
			kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_smwp_enable, DATA_LEN_4, tmp_data, 0);
			kp_himax_in_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, back_data, 4);
		}

		kp_g_core_fp->fp_register_read((*kp_pfw_op)->addr_smwp_enable, DATA_LEN_4, tmp_data, false);
...
static void hx83102ab_set_HSEN_enable(uint8_t HSEN_enable)
{
	uint8_t tmp_data[DATA_LEN_4];

	if (HSEN_enable) {
		kp_himax_in_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
		kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_hsen_enable, DATA_LEN_4, tmp_data, 0);
	} else {
		kp_himax_in_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, tmp_data, 4);
		kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_hsen_enable, DATA_LEN_4, tmp_data, 0);
	}

	/*KI("%s: tmp_data[0]=%d, HSEN_enable=%d, retry_cnt=%d\n", __func__, tmp_data[0],HSEN_enable,retry_cnt);*/
}
...
static void hx83102ab_usb_detect_set(uint8_t *cable_config)
{
	...
	if (cable_config[1] == 0x01) {
		kp_himax_in_parse_assign_cmd(fw_func_handshaking_pwd, tmp_data, 4);
		kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_usb_detect, DATA_LEN_4, tmp_data, 0);
		KI("%s: USB detect status IN!\n", __func__);
	} else {
		kp_himax_in_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, tmp_data, 4);
		kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_usb_detect, DATA_LEN_4, tmp_data, 0);
		KI("%s: USB detect status OUT!\n", __func__);
	}
}
...
static uint8_t hx83102ab_read_DD_status(uint8_t *cmd_set, uint8_t *tmp_data)
{
	...
	kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_dd_handshak_addr, DATA_LEN_4, cmd_set, 0);
	KI("%s: cmd_set[0] = 0x%02X,cmd_set[1] = 0x%02X,cmd_set[2] = 0x%02X,cmd_set[3] = 0x%02X\n",
	  __func__, cmd_set[0], cmd_set[1], cmd_set[2], cmd_set[3]);

	/* Doing hand shaking 0xAA -> 0xBB */
	for (cnt = 0; cnt < 100; cnt++) {
		kp_g_core_fp->fp_register_read((*kp_pfw_op)->addr_dd_handshak_addr, DATA_LEN_4, tmp_data, false);
		usleep_range(10000, 10001);

		if (tmp_data[3] == (*kp_pfw_op)->data_dd_ack[0]) {
			KI("%s Data ready goto moving data\n", __func__);
			break;
		}
		if (cnt >= 99) {
			KI("%s Data not ready in FW\n", __func__);
			return FW_NOT_READY;
		}
	}

	kp_g_core_fp->fp_sense_off(true);
	kp_g_core_fp->fp_register_read((*kp_pfw_op)->addr_dd_data_addr, req_size, tmp_data, false);
...
static int hx83102ab_read_FW_status(uint8_t *state_addr, uint8_t *tmp_addr)
{
	...
	if (state_addr[0] == 0x01) {
		...
		for (i = 0; i < DATA_LEN_4; i++) {
			/* 0x10007F44 */
			state_addr[i + 2] = (*kp_pfw_op)->addr_fw_dbg_msg_addr[i];
			status_addr[i] = (*kp_pfw_op)->addr_fw_dbg_msg_addr[i];
		}

		req_size = 0x04;
		kp_g_core_fp->fp_sense_off(true);
		kp_g_core_fp->fp_register_read(status_addr, req_size, tmp_addr, false);
		kp_g_core_fp->fp_sense_on(0x01);
		...
		for (i = 0; i < DATA_LEN_4; i++) {
			/* 0x900000F8 */
			state_addr[i + 2] = (*kp_pfw_op)->addr_fw_state_addr[i];
			cmd_addr[i] = (*kp_pfw_op)->addr_fw_state_addr[i];
		}

		req_size = 0x30;
		kp_g_core_fp->fp_register_read(cmd_addr, req_size, tmp_addr, false);
	}
...
static void hx83102ab_power_on_init(void)
{
	...
	KI("%s:\n", __func__);
	kp_himax_in_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, tmp_data, 4);
	kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_raw_out_sel, DATA_LEN_4, tmp_data, 0);
	kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_sorting_mode_en, DATA_LEN_4, tmp_data, 0);
...
static int hx83102ab_fts_ctpm_fw_upgrade_with_sys_fs_64k(unsigned char *fw, int len, bool change_iref)
{
	...
	if (kp_g_core_fp->fp_check_CRC((*kp_pfw_op)->addr_program_reload_from, FW_SIZE_64k) == 0)
		burnFW_success = 1;
...
static int hx83102d_0f_esd_check(void)
{
	...
	kp_g_core_fp->fp_register_read((*kp_pzf_op)->addr_sts_chk, DATA_LEN_4, tmp_data, 0);
	...
	kp_g_core_fp->fp_register_read((*kp_pzf_op)->addr_activ_relod, DATA_LEN_4, tmp_data, 0);
...
static void hx83102d_dd_reg_en(bool enable)
{
	...
	kp_g_core_fp->fp_register_write((*kp_pic_op)->adr_osc_pw, DATA_LEN_4, data, 0);

	data[0] = 0xA5;	data[1] = 0x83;
	data[2] = 0x10;	data[3] = 0x2D;
...
static void himax_hx83102d_sense_on(uint8_t FlashMode)
{
	...
	if (!FlashMode) {
		...
		himax_hx83102d_reload_to_active();
		do {
			kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_safe_mode_release_pw,
				sizeof((*kp_pfw_op)->data_safe_mode_release_pw_active), (*kp_pfw_op)->data_safe_mode_release_pw_active, 0);

			kp_g_core_fp->fp_register_read((*kp_pfw_op)->addr_flag_reset_event, DATA_LEN_4, tmp_data, 0);
		...
		if (retry >= 5) {
			...
			KI("%s:OK and Read status from IC = %X,%X\n", __func__, tmp_data[0], tmp_data[1]);
			/* reset code*/
			tmp_data[0] = 0x00;
			...
			kp_g_core_fp->fp_register_write((*kp_pfw_op)->addr_safe_mode_release_pw,
				sizeof((*kp_pfw_op)->data_safe_mode_release_pw_reset), (*kp_pfw_op)->data_safe_mode_release_pw_reset, 0);
		}
	}
}
...
static void hx83102ab_firmware_update_0f(const struct firmware *fw_entry)
{
	...
	kp_g_core_fp->fp_register_write((*kp_pzf_op)->addr_system_reset,  4,  (*kp_pzf_op)->data_system_reset, 0);
	...
	/*bin file start*/
	/*isram*/
	kp_g_core_fp->fp_write_sram_0f(fw_entry, (*kp_pzf_op)->data_sram_start_addr, 0, HX_32K_SZ);
	...
	/*FW config*/
	if ((*kp_G_POWERONOF) == 1)
		kp_g_core_fp->fp_write_sram_0f(fw_entry, (*kp_pzf_op)->data_fw_cfg_1, 0xBFFE, 388);
	else
		kp_g_core_fp->fp_clean_sram_0f((*kp_pzf_op)->data_fw_cfg_1, 388, 1);
	/*ADC config*/
	if ((*kp_G_POWERONOF) == 1)
		kp_g_core_fp->fp_write_sram_0f(fw_entry, (*kp_pzf_op)->data_adc_cfg_1, 0xe000, (128 * 4));
	else
		kp_g_core_fp->fp_clean_sram_0f((*kp_pzf_op)->data_adc_cfg_1, (128 * 4), 2);
...
static void hx83102ab_firmware_read_0f(const struct firmware *fw_entry, int type)
{
	...
	KI("%s, Entering\n", __func__);
	if (type == 0) { /* first 32K */
		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_sram_start_addr, 0, HX_32K_SZ);
		tmp_addr[3] = 0x10;
		tmp_addr[2] = 0x00;
		tmp_addr[1] = 0x68;
		tmp_addr[0] = 0x00;
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_addr, 0x8000, (0x200*4));

		tmp_addr[3] = 0x10;
		tmp_addr[2] = 0x00;
		tmp_addr[1] = 0x73;
		tmp_addr[0] = 0x00;
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_addr, 0x8800, (0x200*4));
		...
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_addr, 0xC178, (84));

		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_fw_cfg_1, 0xBFFE, 388);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_adc_cfg_1, 0xE000, (128*4));
...
static void hx83102d_firmware_read_0f(const struct firmware *fw_entry, int type)
{
	...
	switch (type) {
	case 0:
		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_sram_start_addr, 0, HX_40K_SZ);
		break;
	case 1:
		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_cfg_info, 0xC000, 132);
		break;
	case 2:
		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_fw_cfg_1, 0xC0FE, 484);
		break;
	case 3:
		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_adc_cfg_1, 0xD000, 768);
		break;
	case 4:
		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_adc_cfg_2, 0xD300, 1536);
		break;
	case 5:
		kp_g_core_fp->fp_read_sram_0f(fw_entry, (*kp_pzf_op)->data_adc_cfg_3, 0xE000, 1536);
		break;
	case 6:
		kp_himax_in_parse_assign_cmd(hx83102d_zf_data_bor_prevent_info, tmp_data, 4);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_data, 0xC9E0, 32);
		break;
	case 7:
		kp_himax_in_parse_assign_cmd(hx83102d_zf_data_notch_info, tmp_data, 4);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_data, 0xCA00, 128);
		break;
	case 8:
		kp_himax_in_parse_assign_cmd(hx83102d_zf_func_info_en, tmp_data, 4);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_data, 0xCB00, 12);
		break;
	case 9:
		kp_himax_in_parse_assign_cmd(hx83102d_zf_po_sub_func, tmp_data, 4);
		kp_g_core_fp->fp_read_sram_0f(fw_entry, tmp_data, 0xA000, HX4K);
		break;
	default:
		break;
	}
...
static int hx83102d_0f_overlay(int ovl_type, int mode)
{
	...
	ret = request_firmware(&fwp, fwname, private_ts->dev);
	if (ret < 0) {
	...
	for (i = 1; i < part_num; i++) {
		...
		memcpy(buf, &fwp->data[i * 0x10 + HX64K], 16);

		if (j < 3 && buf[3] == sram_addr[3] && buf[2] == sram_addr[2] && buf[1] == sram_addr[1] && buf[0] == sram_addr[0]) {

			zf_info_arr[j].write_size = buf[7] << 24 | buf[6] << 16 | buf[5] << 8 | buf[4];
			zf_info_arr[j].fw_addr = buf[11] << 24 | buf[10] << 16 | buf[9] << 8 | buf[8];
			/*KI("j = %d; write_size = %d; fw_addr = %d\n", j, zf_info_arr[j].write_size, zf_info_arr[j].fw_addr);*/
			j++;
		}
	}
 ...
 *			KI("%s: self test overlay section 2\n", __func__);
 *		} else if (mode == 1) {
 ...
 *			KI("%s: self test overlay section 0\n", __func__);
 *		}
 *	}
 */
	if (ovl_type == 2) {
		/*gesture overlay when suspend*/
		request = ovl_gesture_request;
		reply = ovl_gesture_reply;
		offset = zf_info_arr[1].fw_addr;
		size = zf_info_arr[1].write_size;
		KI("%s: overlay section 1\n", __func__);
	} else if (ovl_type == 3) {
		/*border overlay when resume*/
		request = ovl_border_request;
		reply = ovl_border_reply;
		offset = zf_info_arr[2].fw_addr;
		size = zf_info_arr[2].write_size;
		KI("%s: overlay section 2\n", __func__);
	} else {
		return HX_INIT_FAIL;
	}
 ...
 *
 *		//g_core_fp.fp_register_write(mpap_addr, DATA_LEN_4, send_data, 0);
 *		count = 0;
 *		do {
 *			g_core_fp.fp_register_write(mpap_addr, DATA_LEN_4, send_data, 0);
 *			g_core_fp.fp_register_read(mpap_addr, DATA_LEN_4, recv_data, 0);
 *		} while ((recv_data[0] != send_data[0] || recv_data[1] != send_data[1] || recv_data[2] != send_data[2] || recv_data[3] != send_data[3]) && count++ < 10);
 *
 *		KI("%s: set mpap pw %d times\n", __func__, count);
 *
 *		//g_core_fp.fp_write_sram_0f(fwp, sram_addr, offset, size);
 *		if (kp_g_core_fp->fp_write_sram_0f_crc(fwp, sram_addr, offset, size) != 0)
 ...
 */
	...
	count = 0;
	do {
		kp_g_core_fp->fp_register_read(handshaking_addr, DATA_LEN_4, recv_data, 0);
	...
	if (count < 10) {
		...
		count2 = 0;
		do {
			kp_g_core_fp->fp_register_write(handshaking_addr, DATA_LEN_4, send_data, 0);
			usleep_range(1000, 1100);
			kp_g_core_fp->fp_register_read(handshaking_addr, DATA_LEN_4, recv_data, 0);
	...
	KI("%s: overlay request %d times; reply %d times\n", __func__, count, count2);
...
static void hx83102a_read_ic_ver(void)
{
	...
	(*kp_ic_data)->vendor_ic_ver = 0xFFFF;
	/* Enable read DD */
	tmp_data[0] = 0x00;
	tmp_data[1] = 0x83;
	tmp_data[2] = 0x10;
	tmp_data[3] = 0x2A;
	kp_himax_in_parse_assign_cmd(hx83102_ic_b9_en, tmp_addr, sizeof(tmp_addr));
	kp_g_core_fp->fp_register_write(tmp_addr,  DATA_LEN_4,  tmp_data,  false);

	/* Read DD data */
	kp_himax_in_parse_assign_cmd(hx83102_ic_fw, tmp_addr, sizeof(tmp_addr));
	kp_g_core_fp->fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
	KI("E8=%X\n", tmp_data[0]);
	(*kp_ic_data)->vendor_ic_ver = tmp_data[0];

	KI("VENDOR_IC_VER : %X\n", ((*kp_ic_data)->vendor_ic_ver));
}
...
static void hx83102b_read_ic_ver(void)
{
	...
	(*kp_ic_data)->vendor_ic_ver = 0xFFFF;
	/* Enable read DD */
	kp_himax_in_parse_assign_cmd(hx83102b_ic_osc_en, tmp_addr, sizeof(tmp_addr));
	kp_g_core_fp->fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
	tmp_data[0] = 0x01;
	kp_g_core_fp->fp_register_write(tmp_addr, DATA_LEN_4, tmp_data, false);
...
static void hx83102d_read_ic_ver(void)
{
	...
	(*kp_ic_data)->vendor_ic_ver = 0xFFFF;
	/* Enable read DD */
	kp_himax_in_parse_assign_cmd(hx83102d_ic_osc_pw, tmp_addr, sizeof(tmp_addr));
	kp_g_core_fp->fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
	tmp_data[0] = 0xA5;
	kp_g_core_fp->fp_register_write(tmp_addr, DATA_LEN_4, tmp_data, false);

	kp_himax_in_parse_assign_cmd(hx83102d_ic_osc_en, tmp_addr, sizeof(tmp_addr));
	kp_g_core_fp->fp_register_read(tmp_addr, DATA_LEN_4, tmp_data, 0);
	tmp_data[0] = 0xDD;
	kp_g_core_fp->fp_register_write(tmp_addr, DATA_LEN_4, tmp_data, false);
...
static void hx83102_dynamic_fw_name(uint8_t ic_name)
{
	...
	if (((*kp_ic_data)->vendor_ic_ver == 0x00) || ((*kp_ic_data)->vendor_ic_ver == 0x21)) { /* Cut 1 */
		memcpy(firmware_name, "Himax_firmware_1.bin", sizeof(char)*strlen("Himax_firmware_1.bin"));
	} else if (((*kp_ic_data)->vendor_ic_ver == 0x01) || ((*kp_ic_data)->vendor_ic_ver == 0x11)) { /* Cut 2 */
...
{
	KI("%s:Entering!\n", __func__);
	kp_himax_in_parse_assign_cmd(hx83102ab_fw_addr_sorting_mode_en, (*kp_pfw_op)->addr_sorting_mode_en, sizeof((*kp_pfw_op)->addr_sorting_mode_en));
	kp_himax_in_parse_assign_cmd(hx83102ab_fw_addr_selftest_addr_en, (*kp_pfw_op)->addr_selftest_addr_en, sizeof((*kp_pfw_op)->addr_selftest_addr_en));
#ifdef HX_ZERO_FLASH
	kp_himax_in_parse_assign_cmd(hx83102ab_data_adc_cfg_1, (*kp_pzf_op)->data_adc_cfg_1, sizeof((*kp_pzf_op)->data_adc_cfg_1));
#endif

	if (ic_name == 0x2a) {
		kp_himax_in_parse_assign_cmd(hx83102a_data_df_rx, (*kp_pdriver_op)->data_df_rx, sizeof((*kp_pdriver_op)->data_df_rx));
		kp_himax_in_parse_assign_cmd(hx83102a_data_df_tx, (*kp_pdriver_op)->data_df_tx, sizeof((*kp_pdriver_op)->data_df_tx));
		kp_himax_in_parse_assign_cmd(hx83102a_data_df_x_res, (*kp_pdriver_op)->data_df_x_res, sizeof((*kp_pdriver_op)->data_df_x_res));
		kp_himax_in_parse_assign_cmd(hx83102a_data_df_y_res, (*kp_pdriver_op)->data_df_y_res, sizeof((*kp_pdriver_op)->data_df_y_res));
	}	else {
		kp_himax_in_parse_assign_cmd(hx83102b_data_df_x_res, (*kp_pdriver_op)->data_df_x_res, sizeof((*kp_pdriver_op)->data_df_x_res));
		kp_himax_in_parse_assign_cmd(hx83102b_data_df_y_res, (*kp_pdriver_op)->data_df_y_res, sizeof((*kp_pdriver_op)->data_df_y_res));
	}
}
...
static void himax_hx83102ab_func_re_init(void)
{
	...
	kp_g_core_fp->fp_fts_ctpm_fw_upgrade_with_sys_fs_64k = hx83102ab_fts_ctpm_fw_upgrade_with_sys_fs_64k;
#ifdef HX_ESD_RECOVERY
...
{
	KI("%s:Entering!\n", __func__);
	kp_himax_in_parse_assign_cmd(hx83102d_fw_addr_raw_out_sel, (*kp_pfw_op)->addr_raw_out_sel, sizeof((*kp_pfw_op)->addr_raw_out_sel));
#if defined(HX_ZERO_FLASH)
	kp_himax_in_parse_assign_cmd(hx83102d_zf_data_sram_start_addr, (*kp_pzf_op)->data_sram_start_addr, sizeof((*kp_pzf_op)->data_sram_start_addr));
	kp_himax_in_parse_assign_cmd(hx83102d_zf_data_adc_cfg_1, (*kp_pzf_op)->data_adc_cfg_1, sizeof((*kp_pzf_op)->data_adc_cfg_1));
	kp_himax_in_parse_assign_cmd(hx83102d_zf_data_adc_cfg_2, (*kp_pzf_op)->data_adc_cfg_2, sizeof((*kp_pzf_op)->data_adc_cfg_2));
	kp_himax_in_parse_assign_cmd(hx83102d_zf_data_adc_cfg_3, (*kp_pzf_op)->data_adc_cfg_3, sizeof((*kp_pzf_op)->data_adc_cfg_3));
#endif
	kp_himax_in_parse_assign_cmd(hx83102d_data_df_x_res, (*kp_pdriver_op)->data_df_x_res, sizeof((*kp_pdriver_op)->data_df_x_res));
	kp_himax_in_parse_assign_cmd(hx83102d_data_df_y_res, (*kp_pdriver_op)->data_df_y_res, sizeof((*kp_pdriver_op)->data_df_y_res));
	kp_himax_in_parse_assign_cmd(hx83102d_data_df_y_res, (*kp_pdriver_op)->data_df_y_res, sizeof((*kp_pdriver_op)->data_df_y_res));
	kp_himax_in_parse_assign_cmd(hx83102d_adr_osc_en, (*kp_pic_op)->adr_osc_en, sizeof((*kp_pic_op)->adr_osc_en));
	kp_himax_in_parse_assign_cmd(hx83102d_adr_osc_pw, (*kp_pic_op)->adr_osc_pw, sizeof((*kp_pic_op)->adr_osc_pw));
}
...
{
	KI("%s:Entering!\n", __func__);
	kp_himax_in_parse_assign_cmd(hx83102e_fw_addr_raw_out_sel, (*kp_pfw_op)->addr_raw_out_sel, sizeof((*kp_pfw_op)->addr_raw_out_sel));
	kp_himax_in_parse_assign_cmd(hx83102e_data_df_rx, (*kp_pdriver_op)->data_df_rx, sizeof((*kp_pdriver_op)->data_df_rx));
	kp_himax_in_parse_assign_cmd(hx83102e_data_df_tx, (*kp_pdriver_op)->data_df_tx, sizeof((*kp_pdriver_op)->data_df_tx));
	kp_himax_in_parse_assign_cmd(hx83102e_data_df_x_res, (*kp_pdriver_op)->data_df_x_res, sizeof((*kp_pdriver_op)->data_df_x_res));
	kp_himax_in_parse_assign_cmd(hx83102e_data_df_y_res, (*kp_pdriver_op)->data_df_y_res, sizeof((*kp_pdriver_op)->data_df_y_res));
	kp_himax_in_parse_assign_cmd(hx83102e_ic_adr_tcon_rst, (*kp_pic_op)->addr_tcon_on_rst, sizeof((*kp_pic_op)->addr_tcon_on_rst));
}
...
static bool hx83102_chip_detect(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		tmp_addr[1] = 0x00;
		tmp_addr[0] = 0xD0;
		if (hx83102_register_read(tmp_addr, DATA_LEN_4, tmp_data) != 0) {
			ret_data = false;
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x10) && ((tmp_data[1] == 0x2a) || (tmp_data[1] == 0x2b) || (tmp_data[1] == 0x2d) || (tmp_data[1] == 0x2e))) {
			if (tmp_data[1] == 0x2a) {
				strlcpy((*kp_private_ts)->chip_name, HX_83102A_SERIES_PWON, 30);
				KI("%s:detect IC HX83102A successfully\n", __func__);
			} else if (tmp_data[1] == 0x2b) {
				strlcpy((*kp_private_ts)->chip_name, HX_83102B_SERIES_PWON, 30);
				KI("%s:detect IC HX83102B successfully\n", __func__);
			} else if (tmp_data[1] == 0x2d) {
				strlcpy((*kp_private_ts)->chip_name, HX_83102D_SERIES_PWON, 30);
				KI("%s:detect IC HX83102D successfully\n", __func__);
			} else {
				strlcpy((*kp_private_ts)->chip_name, HX_83102E_SERIES_PWON, 30);
