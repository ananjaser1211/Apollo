...
static int himax_palm_detect(uint8_t *buf)
{
	...
	if ((ts->power_status == HX_TS_STATE_POWER_ON) && (x == 0xFA5A) && (y == 0xFA5A) && (w == 0x00))
		return PALM_REPORT;
	else
		return NOT_REPORT;
}
...
static ssize_t himax_self_test(struct seq_file *s, void *v)
{
	...
	if (private_ts->power_status != HX_TS_STATE_POWER_ON) {
...
void himax_cable_detect_func(bool force_renew)
{
	...
	if (ts->power_status == HX_TS_STATE_POWER_OFF) {
...
void hx_log_touch_event(struct himax_ts_data *ts)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		if ((((ts->old_finger >> loop_i & 1) == 0)
...
#if defined(HX_SP_INFO) || defined(SEC_PALM_FUNC)
			...
			location_detect(ts, loc, x, y);
#ifndef CONFIG_SAMSUNG_PRODUCT_SHIP
			I("%s: r%d x:%d y:%d loc:%s dd:%d,%d mc:%d tc:%d v:%02X%02X%02X\n", HIMAX_LOG_TAG,
				loop_i, x, y, loc, x - ts->p_x[loop_i], y - ts->p_y[loop_i],
				mc, ts->touch_count, ic_data->vendor_cid_maj_ver,
				ic_data->vendor_panel_ver, ic_data->vendor_cid_min_ver);
#else
			I("%s: r%d loc:%s dd:%d,%d mc:%d tc:%d v:%02X%02X%02X\n",
...
#ifdef HX_SMART_WAKEUP
	if ((ts->power_status == HX_TS_STATE_LPM) && (!FAKE_POWER_KEY_SEND) && (ts->SMWP_enable) && (!hx_touch_data->diag_cmd))
...
#endif
	ts->power_status = HX_TS_STATE_POWER_ON;
...
{
	if (ts->power_status != HX_TS_STATE_POWER_ON) {
	...
	if (ts->SMWP_enable) {
		...
		ts->power_status = HX_TS_STATE_LPM;
	...
	ts->power_status = HX_TS_STATE_POWER_OFF;
