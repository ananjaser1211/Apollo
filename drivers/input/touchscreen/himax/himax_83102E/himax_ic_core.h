...
struct himax_core_fp {
	...
	void (*fp_set_SMWP_enable)(uint8_t SMWP_enable);
	void (*fp_set_HSEN_enable)(uint8_t HSEN_enable);
	void (*fp_diag_register_set)(uint8_t diag_command, uint8_t storage_type);
#ifdef SEC_FACTORY_MODE
	int (*set_edge_border) (int set_val);
//	int (*set_cal_switch) (int set_val);
	int (*get_rport_thrsh) (void);
#endif
