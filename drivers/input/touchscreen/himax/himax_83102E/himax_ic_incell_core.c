...
static int himax_mcu_flash_write_burst_length(uint8_t *reg_byte, uint8_t *write_data, uint32_t length)
{
	uint8_t *data_byte;
	...
	if (cfg_flag == 0) {
...
#ifdef HX_ZERO_FLASH
		max_bus_size = (write_length > HX_MAX_WRITE_SZ - 4)?(HX_MAX_WRITE_SZ - 4):write_length;
#endif
		...
		for (i = 0; i < (total_read_times); i++) {
			...
			if (total_size_temp >= max_bus_size) {
				...
				if (himax_mcu_flash_write_burst_length(tmp_addr, tmp_data, max_bus_size) < 0) {
					I("%s: i2c access fail!\n", __func__);
					return I2C_FAIL;
				}
				...
				if (himax_mcu_flash_write_burst_length(tmp_addr, tmp_data, total_size_temp) < 0) {
					I("%s: i2c access fail!\n", __func__);
					return I2C_FAIL;
				}
			}
...
static int himax_mcu_black_gest_ctrl(bool enable)
{
	...
	I("%s:enable=%d, ts->power_status=%d\n", __func__, enable, private_ts->power_status);

	if (private_ts->power_status == HX_TS_STATE_LPM) {
		if (enable) {
#ifdef HX_RST_PIN_FUNC
			g_core_fp.fp_ic_reset(false, false);
#else
...
static void himax_mcu_set_SMWP_enable(uint8_t SMWP_enable)
{
	uint8_t tmp_data[DATA_LEN_4];
...
static void himax_mcu_set_HSEN_enable(uint8_t HSEN_enable)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t back_data[DATA_LEN_4];
	uint8_t retry_cnt = 0;
...
static bool himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	if (temp_info_data == NULL) {
		E("%s: temp_info_data allocate memory fail!!!!!\n", __func__);
		return false;
	}
...
static void himax_mcu_resend_cmd_func(void)
{

	struct himax_ts_data *ts = private_ts;

#ifdef HX_SMART_WAKEUP
	g_core_fp.fp_set_SMWP_enable(ts->SMWP_enable);
#endif
#ifdef HX_HIGH_SENSE
	g_core_fp.fp_set_HSEN_enable(ts->HSEN_enable);
#endif
...
void himax_mcu_clean_sram_0f(uint8_t *addr, int write_len, int type)
{
	...
	max_bus_size = (write_len > HX_MAX_WRITE_SZ - 4)?(HX_MAX_WRITE_SZ - 4):write_len;
...
int himax_zf_part_info(const struct firmware *fw_entry)
{
	...
	if (G_POWERONOF == 1) {
		...
		if (private_ts->in_self_test == 0) {
			...
			/* ovl_sidx+1 - gesture */
			/* ovl_sidx+2 - border  */
			if (private_ts->power_status == HX_TS_STATE_LPM && private_ts->SMWP_enable)
				ovl_sidx = ovl_sidx + 1;
			else
				ovl_sidx = ovl_sidx + 2;
#else
...
void himax_mcu_firmware_update_0f(const struct firmware *fw_entry)
{
	...
	/* set n frame=0*/
	/*if (G_POWERONOF == 1)
	 *	g_core_fp.fp_write_sram_0f(fw_entry, pfw_op->addr_set_frame_addr, 0xC30C, 4);
	 *else
...
int himax_mcu_0f_operation_dirly(void)
{
	...
	ret = request_firmware(&fw_entry, private_ts->pdata->i_CTPM_firmware_name, private_ts->dev);
	if (ret < 0) {
#if defined(__EMBEDDED_FW__)
		fw_entry = &g_embedded_fw;
		I("%s: Couldn't find userspace FW, use embedded FW(size:%zu) instead.\n", __func__, g_embedded_fw.size);
...
void himax_mcu_0f_operation(struct work_struct *work)
{
	...
	I("%s, Entering\n", __func__);
	I("file name = %s\n", private_ts->pdata->i_CTPM_firmware_name);
	err = request_firmware(&fw_entry, private_ts->pdata->i_CTPM_firmware_name, private_ts->dev);
	if (err < 0) {
#if defined(__EMBEDDED_FW__)
		fw_entry = &g_embedded_fw;
		I("%s: Couldn't find userspace FW, use embedded FW(size:%zu) instead.\n", __func__, g_embedded_fw.size);
...
#endif
#ifdef SEC_FACTORY_MODE
	himax_in_parse_assign_cmd(FW_EDGE_BORDER_ADDR, pfw_op->addr_edge_border, sizeof(pfw_op->addr_edge_border));
//	himax_in_parse_assign_cmd(FW_CAL_ADDR, pfw_op->addr_cal, sizeof(pfw_op->addr_cal));
	himax_in_parse_assign_cmd(FW_RPORT_THRSH_ADDR, pfw_op->addr_rport_thrsh, sizeof(pfw_op->addr_rport_thrsh));
#endif
#endif
#ifdef CORE_FLASH
