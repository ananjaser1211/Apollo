...
#endif
#ifdef CONFIG_CCIC_S2MU107
	struct s2mu107_usbpd_data *usbpd_data = dev_get_drvdata(dev);
#endif
	if (!usbpd_data) {
		pr_err("%s usbpd_data is null!!\n", __func__);
		return -ENODEV;
...
#endif
#ifdef CONFIG_CCIC_S2MU107
		s2mu107_set_normal_mode(usbpd_data);
#endif
	}
#else
	if (mode == 1 || mode == 2) {
...
#endif
#ifdef CONFIG_CCIC_S2MU107
		s2mu107_set_lpm_mode(usbpd_data);
#endif
	}
	else {
#ifdef CONFIG_CCIC_S2MU004
...
#endif
#ifdef CONFIG_CCIC_S2MU107
	s2mu107_control_option_command(usbpd_data, cmd);
#endif
	return size;
...
#endif
#ifdef CONFIG_CCIC_S2MU107
	struct s2mu107_usbpd_data *pdic_data = dev_get_drvdata(dev);
#endif
