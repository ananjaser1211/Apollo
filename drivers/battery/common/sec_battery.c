			      ...
			      bool temp_enable)
{
	if (temp_enable) {
		...
		battery->overheatlimit_threshold_backup =
			battery->overheatlimit_threshold;
		...
		if (battery->overheatlimit_threshold == 990)
			battery->overheatlimit_threshold =
				battery->overheatlimit_threshold_backup;			
	}
}
...
static bool sec_bat_voltage_check(struct sec_battery_info *battery)
{
	...
	if ((battery->status == POWER_SUPPLY_STATUS_FULL) && 
		((battery->charging_mode != SEC_BATTERY_CHARGING_NONE) || (battery->current_event & SEC_BAT_CURRENT_EVENT_HIGH_TEMP_SWELLING))) {
...
*/
	...
	if (fg_reset && bootmode == 2) {
		pr_info("%s: fg_reset(%d) bootmode(%d)\n",
			__func__, fg_reset, bootmode);
		psy_do_property(battery->pdata->fuelgauge_name, set,
			POWER_SUPPLY_PROP_ENERGY_NOW, value);
