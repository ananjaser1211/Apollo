...
static void max77854_fg_read_time(struct max77854_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	int lsb, msb, time;
...
static void max77854_fg_test_print(struct max77854_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	u32 average_vcell;
	u16 w_data;
	...
	reg_data = max77854_read_word(fuelgauge->i2c, FULLCAP_REG);
	pr_info("%s: FULLCAP(%d), data(0x%04x)\n", __func__,
		reg_data/2, reg_data);

	reg_data = max77854_read_word(fuelgauge->i2c, REMCAP_REP_REG);
	pr_info("%s: REMCAP_REP(%d), data(0x%04x)\n", __func__,
		reg_data/2, reg_data);

	reg_data = max77854_read_word(fuelgauge->i2c, REMCAP_MIX_REG);
	pr_info("%s: REMCAP_MIX(%d), data(0x%04x)\n", __func__,
		reg_data/2, reg_data);

	reg_data = max77854_read_word(fuelgauge->i2c, REMCAP_AV_REG);
	pr_info("%s: REMCAP_AV(%d), data(0x%04x)\n", __func__,
		reg_data/2, reg_data);

	reg_data = max77854_read_word(fuelgauge->i2c, CONFIG_REG);
	pr_info("%s: CONFIG_REG(0x%02x), data(0x%04x)\n", __func__,
		CONFIG_REG, reg_data);
#endif

	max77854_fg_read_time(fuelgauge);
}
...
static int max77854_fg_read_vcell(struct max77854_fuelgauge_data *fuelgauge)
{
	...
	if ((fuelgauge->sw_v_empty == VEMPTY_MODE) && vcell > 3550) {
...
#if !defined(CONFIG_SEC_FACTORY)
	max77854_fg_test_print(fuelgauge);
...
static void max77854_fg_low_temp_compensation(struct max77854_fuelgauge_data *fuelgauge, bool en)
{
	u16 data;
	u8 valrt_data[2];
...
int max77854_fg_alert_init(struct max77854_fuelgauge_data *fuelgauge, int soc)
{
	...
	read_data = max77854_read_word(fuelgauge->i2c, (u8)VALRT_THRESHOLD_REG);
	if (read_data != 0xFFA5)
...
static void max77854_fg_set_vempty(struct max77854_fuelgauge_data *fuelgauge, bool en)
{
	if (en) {
		...
		if (val->intval == SEC_FUELGAUGE_CAPACITY_TYPE_RAW) {
			...
			if (fuelgauge->using_hw_vempty) {
				if ((fuelgauge->raw_capacity <= 50) &&
					...
					max77854_fg_set_vempty(fuelgauge, true);
				} else if ((fuelgauge->raw_capacity > 50) &&
					   fuelgauge->hw_v_empty){
					max77854_fg_set_vempty(fuelgauge, false);
				}
			}
			     ...
			     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		...
		if (!fuelgauge->low_temp_compensation_en &&
			...
			max77854_fg_low_temp_compensation(fuelgauge, true);
			...
			max77854_fg_low_temp_compensation(fuelgauge, false);
