...
static int max77865_haptic_set_pwm(void *data, u32 duty)
{
	struct max77865_haptic_drvdata *drvdata = (struct max77865_haptic_drvdata *)data;
...
static int max77865_motor_boost_control(void *data, int control)
{
	struct max77865_haptic_drvdata *drvdata = (struct max77865_haptic_drvdata *)data;

	if (control == BOOST_ON) {
		max77865_update_reg(drvdata->i2c, MAX77865_PMIC_REG_BOOSTCONTROL1,
				0x00, MAX77865_PMIC_REG_BSTOUT_MASK);
		max77865_update_reg(drvdata->i2c, MAX77865_PMIC_REG_BOOSTCONTROL2,
				0x08, MAX77865_PMIC_REG_FORCE_EN_MASK);
		return 0;
	} else if (control == BOOST_OFF) {
		max77865_update_reg(drvdata->i2c, MAX77865_PMIC_REG_BOOSTCONTROL2,
				0x00, MAX77865_PMIC_REG_FORCE_EN_MASK);
		return 0;
	}
	return -1;
}

static int max77865_haptic_i2c(void *data, bool en)
{
	struct max77865_haptic_drvdata *drvdata = (struct max77865_haptic_drvdata *)data;

	return max77865_update_reg(drvdata->i2c,
			MAX77865_PMIC_REG_MCONFIG,
			en ? 0xff : 0x0, MOTOR_EN);
}
...
static int max77865_haptic_probe(struct platform_device *pdev)
{
	...
	shdata->data = drvdata;
	shdata->boost = max77865_motor_boost_control;
	shdata->enable = max77865_haptic_i2c;
	shdata->set_intensity = max77865_haptic_set_pwm;
