		...
		if (kctx->jctx.job_nr > 0) {
			for (i = BASE_JD_ATOM_COUNT-1; i > 0; i--) {
				if (kctx->jctx.atoms[i].status == KBASE_JD_ATOM_STATE_UNUSED)
					...
					dev_warn(dev, " - [%p] Atom %d STATE_HW_COMPLETED\n", &kctx->jctx.atoms[i], i);
					continue;
				}
				else if (kctx->jctx.atoms[i].status == KBASE_JD_ATOM_STATE_COMPLETED) {
					dev_warn(dev, " - [%p] Atom %d STATE_COMPLETED\n", &kctx->jctx.atoms[i], i);
					continue;
				}
				else {
					dev_warn(dev, " - [%p] Atom %d STATE ERR\n", &kctx->jctx.atoms[i], i);
					continue;
				}

				dev_warn(dev, " --  slot_nr 0x%x core_req 0x%x event_code 0x%x gpu_rb_state 0x%x\n",
						      kctx->jctx.atoms[i].slot_nr, kctx->jctx.atoms[i].core_req, kctx->jctx.atoms[i].event_code, kctx->jctx.atoms[i].gpu_rb_state);
				dev_warn(dev, " --  dep Atom %p, Atom %p\n", kctx->jctx.atoms[i].dep[0].atom, kctx->jctx.atoms[i].dep[1].atom);
				...
				if ((kctx->jctx.atoms[i].core_req & BASE_JD_REQ_SOFT_JOB_TYPE) == BASE_JD_REQ_SOFT_FENCE_TRIGGER) {
					if (!kbase_sync_fence_out_info_get(&kctx->jctx.atoms[i], &info_out)) {
						...
						dev_warn(dev, "  **Trigger Fence_out Info [%p] %s: fence type = 0x%x, fence ctx = %llu, fence seqno = %u\n",
								info_out.fence, info_out.name, kctx->jctx.atoms[i].core_req, fence_out->context, fence_out->seqno);
					}
				}
				else if ((kctx->jctx.atoms[i].core_req & BASE_JD_REQ_SOFT_JOB_TYPE) == BASE_JD_REQ_SOFT_FENCE_WAIT) {
					if (!kbase_sync_fence_in_info_get(&kctx->jctx.atoms[i], &info_in)) {
						fence_in = info_in.fence;
						dev_warn(dev, "  **Wait Fence_in Info [%p] %s: fence type = 0x%x, fence ctx = %llu, fence seqno = %u\n",
								info_in.fence, info_in.name, kctx->jctx.atoms[i].core_req, fence_in->context, fence_in->seqno);
					}
				}
				spin_unlock_irqrestore(&kctx->waiting_soft_jobs_lock, lflags);
			}
