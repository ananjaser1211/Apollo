						...
						bool disjoint)
{
	...
	/* MALI_SEC_INTEGRATION */
	KBASE_KTRACE_ADD_JM_SLOT_INFO(kbdev, LSI_KATOM_REMOVED, katom->kctx, katom, 0u, katom->slot_nr, /*info_value*/ action);
					...
					u32 action)
{
	...
	/* MALI_SEC_INTEGRATION */
	if ((action & JS_COMMAND_SW_CAUSES_DISJOINT) ||
			((action & JS_COMMAND_MASK) == JS_COMMAND_HARD_STOP)) {
		int status, clock;
		...
		printk("[%s] gpu power: %d clock: %d action: 0x%X", __func__, status, clock, action);
	}
