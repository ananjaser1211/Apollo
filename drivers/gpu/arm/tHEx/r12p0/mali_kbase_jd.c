...
void kbase_jd_done_worker(struct work_struct *data)
{
	...
	if (!atomic_dec_return(&kctx->work_count)) {
		...
		/* MALI_SEC_INTEGRATION */
		if (kctx->completed_jobs.next == LIST_POISON1 ||
			kctx->completed_jobs.prev == LIST_POISON2) {
			dev_err(kbdev->dev,
					"wrong list on : kctx->completed_jobs - kctx %p, kctx->completed_jobs %p, kctx->completed_jobs.next %p, kctx->completed_jobs.prev %p\n",
					kctx, &kctx->completed_jobs, kctx->completed_jobs.next, kctx->completed_jobs.prev);
		} else {
...
int kbase_jd_init(struct kbase_context *kctx)
{
	...
	/* Specially job_done workqueue priority is changed as HIGH */
	kctx->jctx.job_done_wq = alloc_workqueue("mali_jd", WQ_HIGHPRI | WQ_UNBOUND | __WQ_ORDERED | 0, 1);
