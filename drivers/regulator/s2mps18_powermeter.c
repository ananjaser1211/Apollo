...
static void s2m_adc_read_data2(struct device *dev, int channel);
...
static void s2m_adc_read_data(struct device *dev, int channel)
{
	...
	if (adc_meter->adc_mode == CURRENT_METER) {
		...
		if(channel < 0) {
			for (i = 0; i < 8; i++)
				adc_meter->adc_val[i] = adc_meter->power_val[i];
		} else {
			adc_meter->adc_val[channel] = adc_meter->power_val[channel];
		}
...
void s2mps18_powermeter_init(struct s2mps18_dev *s2mps18)
{
	...
	adc_meter->current_val = kzalloc(sizeof(u16)*S2MPS18_MAX_ADC_CHANNEL, GFP_KERNEL);
	adc_meter->power_val = kzalloc(sizeof(u16)*S2MPS18_MAX_ADC_CHANNEL, GFP_KERNEL);
