...
#ifdef CONFIG_SEC_PM_DEBUG
	ret = s2mps18_read_reg(s2mps18->i2c, S2MPS18_PMIC_REG_PWRONSRC,
			&pmic_onsrc);
	if (ret)
		dev_err(&pdev->dev, "failed to read PWRONSRC\n");

	ret = s2mps18_read_reg(s2mps18->i2c, S2MPS18_PMIC_REG_OFFSRC,
			&pmic_offsrc);
	if (ret)
		dev_err(&pdev->dev, "failed to read OFFSRC\n");

	/* Clear OFFSRC register */
	ret = s2mps18_write_reg(s2mps18->i2c, S2MPS18_PMIC_REG_OFFSRC, 0);
	if (ret)
...
//	s2mps18_write_reg(s2mps18->i2c, S2MPS18_PMIC_REG_PWREN_SEL1, 0xff);
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "Failed to request th140C IRQ: %d: %d\n",
