...
static int manager_notifier_notify(void *data)
{
	...
	if (manager_noti.dest == CCIC_NOTIFY_DEV_USB) {
		...
		if (!manager_noti.sub1 && typec_manager.is_muic_pogo) {
			pr_info("%s: MUIC pogo case.\n", __func__);
			return 0;
		}
...
#if defined(CONFIG_MUIC_SM5504_POGO)
	if (p_noti.id == CCIC_NOTIFY_ID_POGO) {
		...
		case ATTACHED_DEV_POGO_DOCK_49_9K_MUIC:
			...
			if (p_noti.attach) {
				...
				typec_manager.muic_pogo_type = p_noti.cable_type;
				manager_event_work(CCIC_NOTIFY_DEV_MANAGER, CCIC_NOTIFY_DEV_USB,
						CCIC_NOTIFY_ID_USB, CCIC_NOTIFY_ATTACH, USB_STATUS_NOTIFY_ATTACH_DFP, 0);
			} else {
				typec_manager.is_muic_pogo = 0;
				typec_manager.muic_pogo_type = ATTACHED_DEV_NONE_MUIC;
				manager_event_work(CCIC_NOTIFY_DEV_MANAGER, CCIC_NOTIFY_DEV_USB,
						CCIC_NOTIFY_ID_USB, CCIC_NOTIFY_DETACH, USB_STATUS_NOTIFY_DETACH, 0);
			}
			...
			manager_notifier_device_t listener)
{
	...
	if (listener == MANAGER_NOTIFY_CCIC_MUIC ||
		listener == MANAGER_NOTIFY_CCIC_SENSORHUB2) {
	...
	case MANAGER_NOTIFY_CCIC_BATTERY:
		...
		if (typec_manager.is_muic_pogo) {
			m_noti.id = CCIC_NOTIFY_ID_POGO;
			m_noti.sub1 = CCIC_NOTIFY_ATTACH;
			m_noti.sub3 = typec_manager.muic_pogo_type;
			nb->notifier_call(nb, m_noti.id, &(m_noti));
		}
#endif /* CONFIG_MUIC_SM5504_POGO */
		break;
...
int manager_notifier_unregister(struct notifier_block *nb)
{
	...
	if (nb->priority == MANAGER_NOTIFY_CCIC_MUIC ||
		nb->priority == MANAGER_NOTIFY_CCIC_SENSORHUB2) {
