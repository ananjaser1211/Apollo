...
static struct ps5169_data *redrv_data;
static struct device *redriver_device;
...
static int ps5169_power_onoff(void *pdata, bool on)
{
	struct ps5169_data *data = (struct ps5169_data *)pdata;
	...
	if (data->dvdd_vreg_name) {
		if (!data->dvdd_vreg) {
			...
			if (IS_ERR(data->dvdd_vreg)) {
				data->dvdd_vreg = NULL;
				pr_err("%s : failed to get dvdd_vreg %s\n", __func__, data->dvdd_vreg_name);
			}
		}
	}
	...
	if (data->dvdd_vreg) {
		...
		pr_err("%s : dvdd_vreg reg_enabled=%d voltage=%d\n", __func__, reg_enabled, voltage);
	}
	...
	if (on) {
		if (data->dvdd_vreg) {
			if (reg_enabled == 0) {
				...
				if (ret) {
					pr_err("%s : dvdd reg enable fail\n", __func__);
					return ret;
				}
				pr_info("%s : dvdd_vreg turned on\n", __func__);
			}
		}
	} else {
		if (data->dvdd_vreg) {
			if (reg_enabled == 1) {
				...
				if (ret) {
					pr_err("%s : dvdd reg disable fail\n", __func__);
					return ret;
				}
				pr_info("%s : dvdd_vreg turned off\n", __func__);
			}
		}
	}
	pr_info("%s : %s\n", __func__, on ? "on" : "off");
...
int ps5169_config(int config, int owner)
{
	int is_front = 0;
	u8 value;
	...
	pr_info("%s: config(%s)(%d)(%s)\n", __func__, REDRV_MODE_Print[config], owner, (is_front ? "FRONT":"REAR"));
	...
	case USB_ONLY_MODE:
		...
		pr_info("%s: USB3_ONLY_MODE read 0x40 (%x)(%d)(%s)\n",
				__func__, value, enable_state, (is_front ? "FRONT":"REAR"));
	break;
	case DP_ONLY_MODE:
		enable_state |= (1 << owner);
		if (is_front)
		...
		value = i2c_smbus_read_byte_data(redrv_data->i2c, 0x40);
		pr_info("%s: DP4_LANE_MODE read 0x40 (%x)(%d)(%s)\n",
				__func__, value, enable_state, (is_front ? "FRONT":"REAR"));
	break;
	case DP2_LANE_USB_MODE:
		...
		value = i2c_smbus_read_byte_data(redrv_data->i2c, 0x40);
		pr_info("%s: DP2_LANE_USB3_MODE read 0x40 (%x)(%d)(%s)\n",
				__func__, value, enable_state, (is_front ? "FRONT":"REAR"));
	break;
	case CLEAR_STATE:
		enable_state &= ~(1 << owner);
		if (!enable_state) {
			...
			value = i2c_smbus_read_byte_data(redrv_data->i2c, 0x40);
			pr_info("%s: CLEAR_STATE read 0x40 (%x)(%d)(%s)\n",
					__func__, value, enable_state, (is_front ? "FRONT":"REAR"));
		} else {
			pr_info("%s: CLEAR_STATE is not working (%d)\n", __func__, enable_state);
		}
	break;
...
static int ps5169_parse_dt(struct device *dev)
{
	...
	if (of_property_read_string_index(np, "combo,dvdd_vreg_name", 0,
			(const char **)&redrv_data->dvdd_vreg_name)) {
		redrv_data->dvdd_vreg_name = NULL;
		pr_err("%s: Cannot get dvdd_vreg_name\n", __func__);
		return -EINVAL;
	}
	pr_info("%s: dvdd_vreg_name: %s\n", __func__, redrv_data->dvdd_vreg_name);

	redrv_data->con_sel = of_get_named_gpio(np, "combo,con_sel", 0);
	if (redrv_data->con_sel < 0) {
		pr_err("%s: Cannot get con_sel\n", __func__);
		return redrv_data->con_sel;
	}

	redrv_data->redriver_en = of_get_named_gpio(np, "combo,redriver_en", 0);
	if (redrv_data->redriver_en < 0) {
		pr_err("%s: Cannot get redriver_en\n", __func__);
		return redrv_data->redriver_en;
	}
	...
	if (gpio_is_valid(redrv_data->redriver_en)) {
		...
		if (ret < 0) {
			...
			gpio_free(redrv_data->redriver_en);
			return ret;
		}
		ret = gpio_direction_output(redrv_data->redriver_en, 1);
		if (ret < 0) {
			pr_err("%s : gpio %d dir out failed %d\n", __func__, redrv_data->redriver_en, ret);
			gpio_free(redrv_data->redriver_en);
			return ret;
		}
	} else {
