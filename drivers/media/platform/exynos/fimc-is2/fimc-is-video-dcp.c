...
const struct v4l2_file_operations fimc_is_dcpxs_video_fops;
const struct v4l2_ioctl_ops fimc_is_dcpxs_video_ioctl_ops;
const struct vb2_ops fimc_is_dcpxs_qops;

/* Master */
int fimc_is_dcp0s_video_probe(void *data)
{
	int ret = 0;
	...
	ret = fimc_is_video_probe(video,
		FIMC_IS_VIDEO_DCPXS_NAME(0),
		FIMC_IS_VIDEO_DCP0S_NUM,
		VFL_DIR_M2M,
		&core->resourcemgr.mem,
		&core->v4l2_dev,
		&fimc_is_dcpxs_video_fops,
		&fimc_is_dcpxs_video_ioctl_ops);
	if (ret)
...
static int fimc_is_dcpxs_video_open(struct file *file)
{
	int ret = 0;
	int ret_err = 0;
	struct fimc_is_video *video;
	...
	minfo("[D%dS:V] %s\n", device, GET_DCPXS_ID(video), __func__);

	snprintf(name, sizeof(name), "D%dS", GET_DCPXS_ID(video));
	ret = open_vctx(file, video, &vctx, device->instance, FRAMEMGR_ID_DCP0S << GET_DCPXS_ID(video), name);
	if (ret) {
	...
	ret = fimc_is_video_open(vctx,
		device,
		VIDEO_DCPXS_READY_BUFFERS,
		video,
		&fimc_is_dcpxs_qops,
		&fimc_is_ischain_dcp_ops);
	if (ret) {
		merr("fimc_is_video_open is fail(%d)", device, ret);
		goto err_video_open;
	}

	ret = fimc_is_ischain_dcp_open(device, vctx);
	if (ret) {
		merr("fimc_is_ischain_dcp_open is fail(%d)", device, ret);
...
static int fimc_is_dcpxs_video_close(struct file *file)
{
	int ret = 0;
	int refcount;
	...
	ret = fimc_is_ischain_dcp_close(device, vctx);
	if (ret)
		merr("fimc_is_ischain_dcp_close is fail(%d)", device, ret);
...
static unsigned int fimc_is_dcpxs_video_poll(struct file *file,
...
static int fimc_is_dcpxs_video_mmap(struct file *file,
...
const struct v4l2_file_operations fimc_is_dcpxs_video_fops = {
	.owner		= THIS_MODULE,
	.open		= fimc_is_dcpxs_video_open,
	.release	= fimc_is_dcpxs_video_close,
	.poll		= fimc_is_dcpxs_video_poll,
	.unlocked_ioctl	= video_ioctl2,
	.mmap		= fimc_is_dcpxs_video_mmap,
};
...
static int fimc_is_dcpxs_video_querycap(struct file *file, void *fh,
...
static int fimc_is_dcpxs_video_enum_fmt_mplane(struct file *file, void *priv,
...
static int fimc_is_dcpxs_video_get_format_mplane(struct file *file, void *fh,
...
static int fimc_is_dcpxs_video_set_format_mplane(struct file *file, void *fh,
	struct v4l2_format *format)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_dcpxs_video_reqbufs(struct file *file, void *priv,
...
static int fimc_is_dcpxs_video_querybuf(struct file *file, void *priv,
...
static int fimc_is_dcpxs_video_qbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_dcpxs_video_dqbuf(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_dcpxs_video_prepare(struct file *file, void *priv,
	struct v4l2_buffer *buf)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_dcpxs_video_streamon(struct file *file, void *priv,
...
static int fimc_is_dcpxs_video_streamoff(struct file *file, void *priv,
	enum v4l2_buf_type type)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = file->private_data;
...
static int fimc_is_dcpxs_video_enum_input(struct file *file, void *priv,
...
static int fimc_is_dcpxs_video_g_input(struct file *file, void *priv,
	unsigned int *input)
{
...
static int fimc_is_dcpxs_video_s_input(struct file *file, void *priv,
	unsigned int input)
{
	int ret = 0;
	...
	mdbgv_dcpxs("%s(input : %08X)[%d,%d,%d,%d,%d]\n", vctx, __func__, input,
	...
	ret = fimc_is_ischain_dcp_s_input(device, stream, position, vindex, intype, leader);
	if (ret) {
		merr("fimc_is_ischain_dcp_s_input is fail(%d)", vctx, ret);
		goto p_err;
	}
...
static int fimc_is_dcpxs_video_s_ctrl(struct file *file, void *priv,
	struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_IS_FORCE_DONE:
		set_bit(FIMC_IS_GROUP_REQUEST_FSTOP, &device->group_dcp.state);
		break;
	default:
...
static int fimc_is_dcpxs_video_g_ctrl(struct file *file, void *priv,
	struct v4l2_control *ctrl)
{
...
static int fimc_is_dcpxs_video_g_ext_ctrl(struct file *file, void *priv,
...
const struct v4l2_ioctl_ops fimc_is_dcpxs_video_ioctl_ops = {
	.vidioc_querycap		= fimc_is_dcpxs_video_querycap,
	.vidioc_enum_fmt_vid_out_mplane	= fimc_is_dcpxs_video_enum_fmt_mplane,
	.vidioc_g_fmt_vid_out_mplane	= fimc_is_dcpxs_video_get_format_mplane,
	.vidioc_s_fmt_vid_out_mplane	= fimc_is_dcpxs_video_set_format_mplane,
	.vidioc_reqbufs			= fimc_is_dcpxs_video_reqbufs,
	.vidioc_querybuf		= fimc_is_dcpxs_video_querybuf,
	.vidioc_qbuf			= fimc_is_dcpxs_video_qbuf,
	.vidioc_dqbuf			= fimc_is_dcpxs_video_dqbuf,
	.vidioc_prepare_buf		= fimc_is_dcpxs_video_prepare,
	.vidioc_streamon		= fimc_is_dcpxs_video_streamon,
	.vidioc_streamoff		= fimc_is_dcpxs_video_streamoff,
	.vidioc_enum_input		= fimc_is_dcpxs_video_enum_input,
	.vidioc_g_input			= fimc_is_dcpxs_video_g_input,
	.vidioc_s_input			= fimc_is_dcpxs_video_s_input,
	.vidioc_s_ctrl			= fimc_is_dcpxs_video_s_ctrl,
	.vidioc_g_ctrl			= fimc_is_dcpxs_video_g_ctrl,
	.vidioc_g_ext_ctrls		= fimc_is_dcpxs_video_g_ext_ctrl,
};

static int fimc_is_dcpxs_queue_setup(struct vb2_queue *vbq,
...
static void fimc_is_dcpxs_stop_streaming(struct vb2_queue *vbq)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = vbq->drv_priv;
	struct fimc_is_queue *queue;
...
static void fimc_is_dcpxs_buffer_queue(struct vb2_buffer *vb)
{
	int ret = 0;
	...
	ret = fimc_is_ischain_dcp_buffer_queue(device, queue, vb->index);
	if (ret) {
		merr("fimc_is_ischain_dcp_buffer_queue is fail(%d)", device, ret);
		return;
	}
}

static void fimc_is_dcpxs_buffer_finish(struct vb2_buffer *vb)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = vb->vb2_queue->drv_priv;
	...
	ret = fimc_is_ischain_dcp_buffer_finish(device, vb->index);
	if (ret) {
		merr("fimc_is_ischain_dcp_buffer_finish is fail(%d)", device, ret);
		return;
	}
}
