	...
	u32 *indexes)
{
	...
	*hindex |= HIGHBIT_OF(PARAM_3AA_VDMA1_INPUT);
	...
	if (test_bit(FIMC_IS_GROUP_OTF_INPUT, &group->state)) {
		...
		otf_output->width = incrop->w;
		otf_output->height = incrop->h;
		otf_output->crop_offset_x = otcrop->x;
		otf_output->crop_offset_y = otcrop->y;
		otf_output->crop_width = otcrop->w;
		otf_output->crop_height = otcrop->h;
