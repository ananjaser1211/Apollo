...
static int fimc_is_scp_video_open(struct file *file)
{
	...
	snprintf(name, sizeof(name), "SCP");
	ret = open_vctx(file, video, &vctx, device->instance, FRAMEMGR_ID_SCP, name);
	if (ret) {
...
static void fimc_is_scp_stop_streaming(struct vb2_queue *vbq)
{
	int ret = 0;
	struct fimc_is_video_ctx *vctx = vbq->drv_priv;
	struct fimc_is_queue *queue;
