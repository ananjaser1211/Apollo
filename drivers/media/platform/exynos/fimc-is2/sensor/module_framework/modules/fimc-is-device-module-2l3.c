...
#ifdef CONFIG_OF
	fimc_is_module_parse_dt(dev, sensor_2l3_power_setpin);
#endif
...
static const struct of_device_id exynos_fimc_is_sensor_module_2l3_match[] = {
	{
		.compatible = "samsung,sensor-module-2l3",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_sensor_module_2l3_match);

static struct platform_driver sensor_module_2l3_driver = {
	.driver = {
		.name   = "FIMC-IS-SENSOR-MODULE-2L3",
		.owner  = THIS_MODULE,
		.of_match_table = exynos_fimc_is_sensor_module_2l3_match,
	}
};

static int __init fimc_is_sensor_module_2l3_init(void)
{
	int ret;
...
late_initcall(fimc_is_sensor_module_2l3_init);
