...
int sensor_imx616_cis_QuadSensCal_write(struct v4l2_subdev *subdev)
{
	...
	ret = fimc_is_sec_get_cal_buf(&rom_cal_buf, rom_id);
	...
	if (ret < 0) {
		err("cis_imx616 QSC Cal dump fail(%d)!\n", ret);
		goto p_err;
	}
#endif
...
***********************************************************************/
...
#if 1 //SENSOR_IMX616_DEBUG_INFO
	ret = sensor_imx616_cis_get_fineIntegTime(cis);
	if (ret < 0) {
		err("sensor_imx616_cis_get_fineIntegTime fail!! (%d)", ret);
		goto p_err;
	}
	...
	if (sensor_imx616_cal_write_flag == false) {
		...
		if (ret < 0) {
			err("sensor_imx616_Quad_Sens_Cal_write fail!! (%d)", ret);
			goto p_err;
		}
	}
#endif
...
int sensor_imx616_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (cis->rev_flag == true) {
		...
		ret = sensor_imx616_cis_check_rev(cis);
		if (ret < 0) {
			err("sensor_imx616_check_rev fail!!");
			goto p_err;
		}
...
int sensor_imx616_cis_init_3hdr_lsc_table(struct v4l2_subdev *subdev, void *data)
{
	...
	if (ARRAY_SIZE(lsc->ram_table) < table_len) {
		err("ramtable size smaller than %d", table_len);
		goto p_err;
	}
