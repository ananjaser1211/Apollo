...
#define SENSOR_NAME "S5K3M3"
/* #define DEBUG_3M3_PLL */

static const struct v4l2_subdev_ops subdev_ops;

static const u32 *sensor_3m3_global;
static u32 sensor_3m3_global_size;
static const u32 **sensor_3m3_setfiles;
static const u32 *sensor_3m3_setfile_sizes;
...
static void sensor_3m3_set_integration_max_margin(u32 mode, cis_shared_data *cis_data)
{
	...
	switch (mode) {
		...
		case SENSOR_3M3_2016X1134_30FPS:
			...
			break;
		default:
			err("[%s] Unsupport 3m3 sensor mode\n", __func__);
			cis_data->max_margin_coarse_integration_time = SENSOR_3M3_COARSE_INTEGRATION_TIME_MAX_MARGIN;
			dbg_sensor(1, "max_margin_coarse_integration_time(%d)\n",
				cis_data->max_margin_coarse_integration_time);
			break;
...
static void sensor_3m3_cis_data_calculation(const struct sensor_pll_info_compact *pll_info, cis_shared_data *cis_data)
{
	u32 vt_pix_clk_hz = 0;
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_3M3_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_3M3_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_3M3_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time= SENSOR_3M3_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

void sensor_3m3_cis_data_calc(struct v4l2_subdev *subdev, u32 mode)
{
	//int ret = 0;
	...
	if (mode >= sensor_3m3_max_setfile_num) {
...
static int sensor_3m3_wait_stream_off_status(cis_shared_data *cis_data)
{
	int ret = 0;
	u32 timeout = 0;
...
int sensor_3m3_cis_log_status(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
...
static int sensor_3m3_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	struct i2c_client *client = NULL;
...
#else
static inline int sensor_3m3_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{ return 0; }
#endif
 ...
 */
int sensor_3m3_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	ret = sensor_3m3_cis_group_param_hold_func(subdev, hold);
	if (ret < 0)
		goto p_err;
...
int sensor_3m3_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	...
	ret = sensor_cis_set_registers(subdev, sensor_3m3_global, sensor_3m3_global_size);
	if (ret < 0) {
		err("sensor_3m3_set_registers fail!!");
		goto p_err;
	}
...
int sensor_3m3_cis_update_crop_region(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u16 x_start = 0;
	u16 y_start = 0;
...
static void sensor_3m3_cis_set_paf_stat_enable(u32 mode, cis_shared_data *cis_data)
{
	WARN_ON(!cis_data);
...
int sensor_3m3_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
	struct fimc_is_device_sensor *device;
	...
	I2C_MUTEX_LOCK(cis->i2c_lock);
	ret = sensor_cis_set_registers(subdev, sensor_3m3_setfiles[mode], sensor_3m3_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_3m3_set_registers fail!!");
		goto p_err;
	}
	...
	if (device->image.framerate == 24
		...
		fimc_is_sensor_write16(client, 0x30DA, 0x0016);
		fimc_is_sensor_write16(client, 0x30DC, 0x0016);
	}
#endif

#ifdef CAMERA_REAR2_SENSOR_SHIFT_CROP
	if (device->position == SENSOR_POSITION_REAR2) {
		ret = sensor_3m3_cis_update_crop_region(subdev);
		if (ret < 0) {
			err("sensor_3m3_cis_update_crop_region fail!!");
			goto p_err;
		}
	}
#endif
...
int sensor_3m3_cis_set_size(struct v4l2_subdev *subdev, cis_shared_data *cis_data)
{
	int ret = 0;
	bool binning = false;
	...
	ret = sensor_3m3_wait_stream_off_status(cis_data);
	if (ret) {
		err("Must stream off\n");
		ret = -EINVAL;
		goto p_err;
	...
	if (binning) {
		ratio_w = (SENSOR_3M3_MAX_WIDTH / cis_data->cur_width);
		ratio_h = (SENSOR_3M3_MAX_HEIGHT / cis_data->cur_height);
	} else {
		ratio_w = 1;
		ratio_h = 1;
	}

	if (((cis_data->cur_width * ratio_w) > SENSOR_3M3_MAX_WIDTH) ||
		((cis_data->cur_height * ratio_h) > SENSOR_3M3_MAX_HEIGHT)) {
	...
	start_x = ((SENSOR_3M3_MAX_WIDTH - cis_data->cur_width * ratio_w) / 2) & (~0x1);
	start_y = ((SENSOR_3M3_MAX_HEIGHT - cis_data->cur_height * ratio_h) / 2) & (~0x1);
...
int sensor_3m3_cis_stream_on(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_3m3_cis_stream_off(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_3m3_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	hold = sensor_3m3_cis_group_param_hold_func(subdev, 0x01);
	if (hold < 0) {
		ret = hold;
		goto p_err;
	}
...
p_err:
	if (hold > 0) {
		hold = sensor_3m3_cis_group_param_hold_func(subdev, 0x00);
		if (hold < 0)
			ret = hold;
	}
...
int sensor_3m3_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis = NULL;
...
int sensor_3m3_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_3m3_cis_adjust_frame_duration(struct v4l2_subdev *subdev,
...
int sensor_3m3_cis_set_frame_duration(struct v4l2_subdev *subdev, u32 frame_duration)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_3m3_cis_set_frame_rate(struct v4l2_subdev *subdev, u32 min_fps)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
	...
	ret = sensor_3m3_cis_set_frame_duration(subdev, frame_duration);
	if (ret < 0) {
...
int sensor_3m3_cis_adjust_analog_gain(struct v4l2_subdev *subdev, u32 input_again, u32 *target_permile)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_3m3_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_3m3_cis_get_analog_gain(struct v4l2_subdev *subdev, u32 *again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_3m3_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_3m3_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_3m3_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	/* DGain WDR Mode */
	if (fimc_is_vender_wdr_mode_on(cis_data)) {
		fimc_is_sensor_write16(client, 0x31FC, 0x0001);
	} else {
		fimc_is_sensor_write16(client, 0x31FC, 0x0000);
	}
	...
	/* Long digital gain */
	if (fimc_is_vender_wdr_mode_on(cis_data)) {
		ret = fimc_is_sensor_write16(client, 0x3062, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_3m3_cis_get_digital_gain(struct v4l2_subdev *subdev, u32 *dgain)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_3m3_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
	...
	/* 3m3 cannot read min/max digital gain */
...
int sensor_3m3_cis_set_adjust_sync(struct v4l2_subdev *subdev, u32 sync)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	struct i2c_client *client;
...
int sensor_3m3_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct fimc_is_cis *cis;
	cis_shared_data *cis_data;
...
static struct fimc_is_cis_ops cis_ops = {
	...
	.cis_log_status = sensor_3m3_cis_log_status,
	.cis_group_param_hold = sensor_3m3_cis_group_param_hold,
	.cis_set_global_setting = sensor_3m3_cis_set_global_setting,
	.cis_mode_change = sensor_3m3_cis_mode_change,
	.cis_set_size = sensor_3m3_cis_set_size,
	.cis_stream_on = sensor_3m3_cis_stream_on,
	.cis_stream_off = sensor_3m3_cis_stream_off,
	.cis_set_exposure_time = sensor_3m3_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_3m3_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_3m3_cis_get_max_exposure_time,
	.cis_adjust_frame_duration = sensor_3m3_cis_adjust_frame_duration,
	.cis_set_frame_duration = sensor_3m3_cis_set_frame_duration,
	.cis_set_frame_rate = sensor_3m3_cis_set_frame_rate,
	.cis_adjust_analog_gain = sensor_3m3_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_3m3_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_3m3_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_3m3_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_3m3_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_3m3_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_3m3_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_3m3_cis_get_min_digital_gain,
	.cis_get_max_digital_gain = sensor_3m3_cis_get_max_digital_gain,
	...
	const struct i2c_device_id *id)
{
	...
	sensor_peri = find_peri_by_cis_id(device, SENSOR_NAME_S5K3M3);
	if (!sensor_peri) {
	...
	if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setA") == 0) {
		...
		sensor_3m3_global_size = ARRAY_SIZE(sensor_3m3_setfile_A_Global);
		sensor_3m3_setfiles = sensor_3m3_setfiles_A;
		sensor_3m3_setfile_sizes = sensor_3m3_setfile_A_sizes;
		sensor_3m3_pllinfos = sensor_3m3_pllinfos_A;
		sensor_3m3_max_setfile_num = ARRAY_SIZE(sensor_3m3_setfiles_A);
	}
#if 0
	else if (strcmp(setfile, "setB") == 0) {
		probe_info("%s setfile_B\n", __func__);
		sensor_3m3_global = sensor_3m3_setfile_B_Global;
		sensor_3m3_global_size = ARRAY_SIZE(sensor_3m3_setfile_B_Global);
		sensor_3m3_setfiles = sensor_3m3_setfiles_B;
		sensor_3m3_setfile_sizes = sensor_3m3_setfile_B_sizes;
		sensor_3m3_pllinfos = sensor_3m3_pllinfos_B;
		sensor_3m3_max_setfile_num = ARRAY_SIZE(sensor_3m3_setfiles_B);
	}
#endif
	else {
...
static const struct of_device_id sensor_cis_3m3_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-3m3",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_cis_3m3_match);

static const struct i2c_device_id sensor_cis_3m3_idt[] = {
	{ SENSOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_cis_3m3_driver = {
	.probe	= cis_3m3_probe,
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_cis_3m3_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_cis_3m3_idt
};

static int __init sensor_cis_3m3_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_3m3_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_3m3_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_cis_3m3_init);
