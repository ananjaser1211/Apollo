...
#include "s6e3fa9_anote_param.h"
#include "dd.h"

#if defined(CONFIG_EXYNOS_DECON_MDNIE)
#include "mdnie.h"
#include "s6e3fa9_anote_mdnie.h"
#endif

#include "s6e3fa9_anote_selfmask.h"

#if defined(CONFIG_DISPLAY_USE_INFO)
#include "dpui.h"

#define	DPUI_VENDOR_NAME	"SDC"
#define DPUI_MODEL_NAME		"AMS667UK08"
#endif
...
#endif
	unsigned int			green_circle_state;
...
int dsim_write_img_sd(struct lcd_info *lcd, const u8 *buf, int size)
{
	...
	do {
		...
		ret = dsim_write_data(lcd->dsim, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)cmdbuf, tx_size + 1);
		if (ret < 0) {
			dsim_err("%s:failed to write command %d, size %d remain %d\n",
		...
		len += tx_size;
		remained -= tx_size;
		dsim_info("%s tx_size %d len %d, remained %d\n",
...
#if defined(CONFIG_EXYNOS_DECON_MDNIE) || defined(CONFIG_SUPPORT_DOZE)
...
static int s6e3fa9_enable_self_mask(struct lcd_info *lcd)
{
	int ret = 0;

	DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	DSI_WRITE(SEQ_SELF_MASK_ENABLE, ARRAY_SIZE(SEQ_SELF_MASK_ENABLE));
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
...
static int s6e3fa9_disable_self_mask(struct lcd_info *lcd)
{
	int ret = 0;

	DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	DSI_WRITE(SEQ_SELF_MASK_DISABLE, ARRAY_SIZE(SEQ_SELF_MASK_DISABLE));
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
...
static int s6e3fa9_enable_green_circle(struct lcd_info *lcd)
{
	int ret = 0;

	DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
#if defined(CONFIG_SEC_FACTORY)
	DSI_WRITE(SEQ_SELF_MASK_GREEN_CIRCLE_ON_FACTORY, ARRAY_SIZE(SEQ_SELF_MASK_GREEN_CIRCLE_ON_FACTORY));
#endif
	DSI_WRITE(SEQ_SELF_MASK_GREEN_CIRCLE_ON, ARRAY_SIZE(SEQ_SELF_MASK_GREEN_CIRCLE_ON));
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
	dev_info(&lcd->ld->dev, "%s\n", __func__);
...
static int s6e3fa9_disable_green_circle(struct lcd_info *lcd)
{
	int ret = 0;

	DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	DSI_WRITE(SEQ_SELF_MASK_GREEN_CIRCLE_OFF, ARRAY_SIZE(SEQ_SELF_MASK_GREEN_CIRCLE_OFF));
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
...
static int panel_set_green_circle(struct lcd_info *lcd)
{
	int ret = 0;

	if (lcd->green_circle_state == GREEN_CIRCLE_ON)
		s6e3fa9_enable_green_circle(lcd);
	else
		s6e3fa9_disable_green_circle(lcd);
	return ret;
}
...
static int s6e3fa9_read_eareg(struct lcd_info *lcd)
{
	int ret = 0;

	ret = panel_read_bit_info(lcd, LDI_BIT_ENUM_DSIERR, NULL);
	if (ret < 0)
		dev_info(&lcd->ld->dev, "%s: fail\n", __func__);
...
static int s6e3fa9_exit(struct lcd_info *lcd)
{
	...
	s6e3fa9_read_eareg(lcd);		// for debug
...
static int s6e3fa9_init(struct lcd_info *lcd)
{
	...
	DSI_WRITE(SEQ_PCD_DET_LOW, ARRAY_SIZE(SEQ_PCD_DET_LOW));
	DSI_WRITE(SEQ_PARTIAL_UPDATE, ARRAY_SIZE(SEQ_PARTIAL_UPDATE));
	DSI_WRITE(SEQ_FFC_SET, ARRAY_SIZE(SEQ_FFC_SET));
	DSI_WRITE(SEQ_SELF_MASK_SD_PATH, ARRAY_SIZE(SEQ_SELF_MASK_SD_PATH));
	dsim_write_img_sd(lcd, S6E3FA9_SELF_MASK_IMG, ARRAY_SIZE(S6E3FA9_SELF_MASK_IMG));
	DSI_WRITE(SEQ_SELF_MASK_SD_PATH_SET, ARRAY_SIZE(SEQ_SELF_MASK_SD_PATH_SET));
	DSI_WRITE(SEQ_SELF_MASK_ENABLE, ARRAY_SIZE(SEQ_SELF_MASK_ENABLE));
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
...
static int s6e3fa9_probe(struct lcd_info *lcd)
{
	...
	lcd->green_circle_state = GREEN_CIRCLE_OFF;
...
static DEVICE_ATTR(fp_green_circle, 0664, fp_green_circle_show, fp_green_circle_store);
...
static int dsim_panel_mask_brightness(struct dsim_device *dsim)
{
	...
	if (regs->mask_layer == false && decon->current_mask_layer == true) {
		...
		decon_reg_start(decon->id, &psr);
		decon_reg_wait_for_update_timeout(decon->id, SHADOW_UPDATE_TIMEOUT); /* wait for frame start */
